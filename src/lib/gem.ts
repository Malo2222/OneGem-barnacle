...
export const PLATFORMS: {
  ...
  { id: "imessage", label: "iMessage", short: "iM", color: "oklch(0.72 0.19 145)" },
  { id: "sms", label: "SMS", short: "SMS", color: "oklch(0.72 0.15 230)" },
  { id: "instagram", label: "Instagram", short: "IG", color: "oklch(0.68 0.22 15)" },
  { id: "snapchat", label: "Snapchat", short: "SC", color: "oklch(0.86 0.17 100)" },
...
export function parseCapture(raw: string): {
  ...
  const lines = text
    ...
    .map((l) => l.trim())
    .filter(Boolean)
    .filter((l) => !/^(instagram|snapchat|messages|imessage|mail|gmail|now|\d+m ago)$/i.test(l));

  const joined = lines.join("\n");
  const colon = joined.match(/^([^:\n]{1,40}):\s*([\s\S]+)$/);
  if (colon) return { sender: colon[1]!.trim(), body: colon[2]!.trim(), platform };

  const snap = joined.match(/^(.{1,40}?)\s+(sent you a snap|sent you a chat|sent a message)/i);
  if (snap) return { sender: snap[1]!.trim(), body: joined, platform: "snapchat" };
